...
public class SelectRoomHandler extends SimpleChannelInboundHandler<MessageUpProto.MessageUp> {
    ...
    protected void messageReceived(ChannelHandlerContext ctx, MessageUpProto.MessageUp message) throws Exception {
        if (message.getHeader().getMsgType() == Protocol.TYPE_SELECT_ROOM) {
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        ...
        GameUpBuffer msg = Utils.packgeUpData(ctx.attr(Constans.conn).get(), Protocol.TYPE_SELECT_ROOM, -1, SelectProtocol.DESTORY_BRO, null);
